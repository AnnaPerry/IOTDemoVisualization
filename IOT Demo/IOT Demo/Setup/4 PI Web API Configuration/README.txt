...
Additionally, since this application depends on the PI Web API service running, you can create a Windows Scheduled Task to check once eveyr minute and start the PI Web API Service if it is stopped.  To do this, use the included shortcut to launch the Task Scheduler program.  Then, in the right-hand Actions menu, click "Import Task...", then navigate to select the included sample task file named "Task to Automatically Start PI Web API.xml" that is included in this folder.  After the import has run, click "Change User or Group..." button and specify an administrator-privledged account that will be used to run this task.  After selecting that user, below, click that you want this task to "Run whether user is logged on or not", and below that, click "Run with highest priveledges".

After doing so, click "OK" to finish creating the task, then enter in the password for that administrator-priveledged account.  With these steps done, the PI Web API service will now automatically be restarted every 60 seconds if it is detected to be stopped.